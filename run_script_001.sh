...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nnodes=1 --nproc-per-node=4 \
...
--batch_size_training 8 --val_batch_size 8 --gradient_accumulation_steps 4 \
--dist_checkpoint_root_folder ~/llama-recipes/outputs/llama2/ --dist_checkpoint_folder llama2 --batching_strategy='padding' \
--dataset opnqa_steering_dataset  --output_dir /rscratch/data/steerable_pluralism/lora/${DATASET_PATH}/${STEERING_TYPE}_Meta-Llama-3-8B \
--name ${DATASET_PATH}_${STEERING_TYPE}_${MODEL_NICKNAME} --lr 4e-4 --num_epochs 10 --loss_function_type wd --dataset_path ${DATASET_PATH} --steering_type ${STEERING_TYPE} --model_name ${MODEL_NAME}
